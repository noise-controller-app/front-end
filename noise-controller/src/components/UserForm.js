...
export default withFormik({
    ...
    validationSchema: Yup.object().shape({
        name: Yup.string().min(6).required('We would love to get to know you better. Maybe just start with your name? :)'),
        email: Yup.string().email('Invalid email address.').required('How are we supposed to send you our newsletter without your email?'),
        password: Yup.string().min(7).required('Password required.'),
        classroom: Yup.string().required("C'mon, you're a teacher... You don't have a classroom?")
